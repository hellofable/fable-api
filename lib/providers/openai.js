...
export async function generateSynopsis({ systemPrompt, prompt, maxTokens = 150, temperature = 0.7 }) {
  ...
  console.log('[OpenAI Provider] Request:', { model: OPENAI_MODEL, messages });
  ...
  if (!synopsis) {
    ...
    try {
      ...
      console.error('[OpenAI Provider] Fallback completion failed', fallbackErr);
    }
  }

  if (!synopsis) {
    console.error('[OpenAI Provider] Empty synopsis after fallback. Raw response for debugging will follow.');
